...
{
    auto newNode = std::make_unique<TreeNode>(m_itemData, m_parentNode);
    // Deep copy (recursive)
    std::transform(m_childNodes.begin(), m_childNodes.end(), std::back_inserter(newNode->m_childNodes),
                   [&](const std::unique_ptr<TreeNode> &child) {
                       auto newChild = child->clone();
                       newChild->m_parentNode = newNode.get();
                       return newChild;
                   });
    return newNode;
}
